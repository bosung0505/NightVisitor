...
## 29. 고급 뮤턴트 기믹 확장 및 시스템 안정화 (2026.04.12 추가)

### 디코이(미끼) 뮤턴트 기믹 구현 (`MutantAI.cs`)
* **목적지 분리:** `isDecoy`를 체크하고 목적지 큐브의 이름을 지정하면, 마을이 아닌 해당 위치로 조용히 걸어가다 맵에서 사라집니다. '왜 저기로 가지?' 하는 궁금증을 유발하는 배경용 역할입니다.
* **심리전(함정) 유도:** 만약 플레이어가 이 미끼를 쏘아버리면 기존에 구현된 **연쇄 어그로(Chain Aggro)** 방사 시스템이 강제로 발동하여, 맵 곳곳에 있던 다른 돌연변이들이 분노하여 일제히 플레이어를 향해 덮쳐오는 치명적인 패널티를 겪게 됩니다.

### 무한 반복 웨이브(Loop Waves) 시스템 (`StageSelectManager.cs`, `MutantSpawner.cs`)
* 타이머 서바이벌 맵(Map 2)에서 설정해둔 스폰 웨이브가 끝나버려 06:00 종료 시간까지 텅 비는 문제를 완벽히 해결하기 위해, `StageConfig2`에 **`loopWaves`** 옵션을 추가했습니다.
* 이 옵션이 켜져 있으면 짜여진 스폰 루틴(예: 0~5그룹)이 끝난 뒤 10초 휴식(숨고르기)을 가지고, 자동으로 0번 그룹부터 다시 그 타임라인 패턴 그대로 무한 반복 스폰을 시작합니다.

### 기습 달리기 모드 & 그룹 단위 개별 난이도 주입 (`MutantSpawner.cs`)
* 기존에 스테이지 탭 단위로 넓게 묶여있던 반쪽짜리 통합 '부활(`enableRevive`)' 옵션을 떼어내어, 가장 세밀한 단위인 **스폰 그룹(`MutantSpawnGroup`)별 개별 옵션**으로 하향 이관했습니다. 
* 마을 입구에서 가다 서다를 반복하는 산책 지연 전략을 완전히 무시하고, 스폰되자마자 카메라 밑(결승점)을 향해 냅다 전력 질주하는 **`startRunning`** 기능도 스폰 그룹 단위 옵션으로 추가했습니다.
* 이를 통해 특정 웨이브에서만 "죽어도 무조건 기어오며, 생성되자마자 전력 런다운을 꽂는 특수 돌연변이 개체군"을 몰아넣는 고차원적인 레벨 디자인(난이도 그라데이션) 맵 세팅이 가능해졌습니다.

### 몬스터 AI 버그 픽스 - 어그로 목적지 직진 오류
* 플레이어에게 맞지 않고 단순히 동료가 죽는 소리(연쇄 어그로)에만 끌렸을 때(`hitCount == 0`), 두뇌 페이즈는 '플레이어 추적(Chase)' 상태로 들어가지만 실제 내비게이션 목적지는 바뀌지 않고 기존 마을 큐브를 향해 미친 듯이 질주하던 심각한 물리 오류를 해결했습니다.
* 거리 및 추적 판정문 기준점 자체를 주먹구구식 변수인 '초기 누적 맞은 횟수(`hitCount`)'에서 전면 배제하고, 몬스터의 '명확한 현재 상태(`currentPhase`)' 논리만으로 목적지 변경 로직이 자연스럽게 스위칭되도록 `Update()` 루프를 전면 교정했습니다. 이를 통해 어떤 식으로 어그로에 끌리든 즉각 플레이어에게 고개를 홱 틀고 덮쳐오게끔 안정화되었습니다.

---

## 30. 다중 스테이지 UI 탭 관리 및 결과 시스템 완성 (2026.04.12 추가)

### 다중 UI 탭 컨트롤러 (`MissionPanelController.cs`)
* **기획 배경:** 스테이지 수가 확장됨에 따라 수십 개의 스테이지 버튼 `OnClick()` 이벤트에 일일이 다른 패널을 끄는 `SetActive(false)`를 지정해야 하는 인스펙터 노가다 문제를 단숨에 해결하기 위해 추가된 헬퍼 스크립트입니다.
* **배열 기반 자동 토글:** 모든 스테이지 패널을 배열에 순서대로 넣고 `ActivatePanel(int index)` 함수를 통해 클릭된 번호의 패널만 켜고 나머지는 자동으로 꺼주어 작업 소요 시간이 획기적으로 줄었습니다.

### Map 2 전용 독립 결과 시스템 (`Map2ResultManager.cs`)
* **기획 배경:** 맵 1(Kill 카운트 의존)과 로직이 완전히 분리된 맵 2만의 서바이벌 생존 성공 여부 및 베이스 침공, 사망 등 각 뷰(View)에 대응하는 독자적인 결과 UI 매니저를 도입했습니다.
* **시간 동결 및 UI 연출 통일:** 
  * 패널 호출 시 `Time.timeScale = 0` 및 `AudioListener.pause = true`를 선언하여 화면 정지뿐 아니라 진행되던 모든 소리(사격음, 빗소리, 발소리 등 글로벌 오디오)도 함께 동결시킵니다.
  * 맵 1과 동일하게 **DOTween 페이드 인/아웃** 로직을 적용하여, Return Button 클릭 시 패널이 남지 않고 부드럽게 화면 밖으로 스며들듯 사라지고 사운드가 복원되도록 연출을 강화했습니다.

### 러너형(Runner) 좀비 물리 개선 (`MutantAI.cs`)
* 마을 외곽으로 전력질주하던 몬스터가 피격 시 카메라 쪽으로 목표를 바꾸면서, 기존에 달리던 **NavMesh 관성** 탓에 마치 자동차가 원심력으로 부드럽게 코너링(Steering)을 하듯 크게 빙글 도는 부자연스러운 현상을 타파했습니다.
* **직각 턴 기믹(`HitScreamAndTurnRoutine`):** 타겟이 변환될 때 `agent.velocity = Vector3.zero;` 와 `updateRotation = false;`를 강제로 꽂아 엔진 가속도를 완전히 멸종시켰습니다. 이제 즉각 그 자리에 멈춰 서서 플레이어를 똑바로 쳐다보고 소름 끼치게 울부짖은 뒤 직선으로 달려옵니다.

### 환경(날씨) 동적 제어 시스템 (`StageSelectManager.cs`)
* **스테이지 별 비(Rain) 토글 기능:** `StageConfig`와 `StageConfig2`에 **`Enable Rain`** 체크박스를 신설하여, 각 스테이지의 분위기나 콘셉트에 따라 날씨를 자유자재로 설정할 수 있게 했습니다.
* 코드 레벨에서 메인 카메라에 붙박아둔 파티클 오브젝트(`RainParticles`)를 찾아 온/오프하며, 그 안의 **Audio Source(비 소리)**도 컴포넌트로 캐싱해 파티클이 활성화/비활성화되는 것에 맞춰 사운드도 즉각 `Play()/Stop()` 연동되도록 깔끔하게 설계했습니다.
